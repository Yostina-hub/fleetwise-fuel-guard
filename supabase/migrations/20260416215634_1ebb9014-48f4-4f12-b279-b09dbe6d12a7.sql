...
BEGIN
  ...
  -- Step 3: Fulfill within 5% tolerance → clearance = 'cleared'
  BEGIN
    UPDATE public.fuel_requests SET
      status = 'fulfilled',
      actual_liters = 51,
  ...
  -- Step 4: Deviation > 5% triggers deviation_detected
  BEGIN
    UPDATE public.fuel_requests SET
      actual_liters = 60,
      actual_cost = 5400
  ...
  -- Step 5: Schema integrity (vehicle + generator enterprise fields)
  BEGIN
  ...
  -- Step 6: Oracle-style work order header fields
  BEGIN
    PERFORM 1 FROM information_schema.columns
  ...
  -- Step 7: Approval RPCs (delegation matrix)
  BEGIN
  ...
  -- Cleanup: unlink first, then delete WO, then delete request
  IF v_wo_id IS NOT NULL THEN
