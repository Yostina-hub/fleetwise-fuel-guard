...
BEGIN
  ...
  BEGIN
    ...
    ) VALUES (v_org, v_wo_id, 1, 'fleet_manager', v_actor, 'pending')
  ...
  BEGIN
    ...
    ) VALUES (v_org, v_wo_id, 7500, v_actor, v_actor, 'ops_manager', 'pending')
    RETURNING id INTO v_emoney_approval_id;
    UPDATE public.fuel_work_orders SET emoney_initiated = true, emoney_amount = 7500, updated_at = now() WHERE id = v_wo_id;
  ...
  BEGIN
    ...
    FROM public.get_nearby_fuel_stations(9.0192::numeric, 38.7525::numeric, 500::numeric, 'diesel'::text, 0::numeric);
    IF v_nearby_count > 0 THEN
      t_step:='step10_nearby_stations'; t_flow:='steps_6_12'; t_status:='PASS'; t_detail:=v_nearby_count||' stations within 500km'; RETURN NEXT;
    ELSE
      t_step:='step10_nearby_stations'; t_flow:='steps_6_12'; t_status:='WARN'; t_detail:='No stations'; RETURN NEXT;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    t_step:='step10_nearby_stations'; t_flow:='steps_6_12'; t_status:='FAIL'; t_detail:=SQLERRM; RETURN NEXT;
  END;

  -- Step 11: Fulfillment within tolerance (≤5%) + actual_cost so txn can be created
  BEGIN
    UPDATE public.fuel_requests
    SET status = 'fulfilled', actual_liters = 102, actual_cost = 7650,
        station_id = v_station_id, fulfilled_at = now()
    WHERE id = v_request_id;

    SELECT id INTO v_txn_id FROM public.fuel_transactions
    WHERE vehicle_id = v_vehicle AND organization_id = v_org
      AND notes LIKE 'FR:' || v_request_id::text || '%'
    ORDER BY created_at DESC LIMIT 1;

    IF v_txn_id IS NOT NULL THEN
      t_step:='step11_transaction'; t_flow:='steps_6_12'; t_status:='PASS'; t_detail:='txn '||v_txn_id::text; RETURN NEXT;
    ELSE
      t_step:='step11_transaction'; t_flow:='steps_6_12'; t_status:='FAIL'; t_detail:='no txn auto-created'; RETURN NEXT;
    END IF;
  EXCEPTION WHEN OTHERS THEN
  ...
  BEGIN
    ...
    IF v_clearance_status IN ('cleared','deviation_detected') THEN
      t_step:='step12_clearance'; t_flow:='steps_6_12'; t_status:='PASS'; t_detail:='clearance='||v_clearance_status; RETURN NEXT;
    ELSE
      t_step:='step12_clearance'; t_flow:='steps_6_12'; t_status:='FAIL'; t_detail:='clearance='||COALESCE(v_clearance_status,'null'); RETURN NEXT;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    t_step:='step12_clearance'; t_flow:='steps_6_12'; t_status:='FAIL'; t_detail:=SQLERRM; RETURN NEXT;
  END;

  -- Step 12.1: Force a deviation by switching status to pending then refulfilling with high actual
  BEGIN
    UPDATE public.fuel_requests SET status = 'approved', clearance_status = NULL, fulfilled_at = NULL WHERE id = v_request_id;
    UPDATE public.fuel_requests
    SET status = 'fulfilled', actual_liters = 130, actual_cost = 9750,
        fulfilled_at = now()
    WHERE id = v_request_id;
    ...
    IF v_clarification_id IS NULL THEN
      ...
      ) VALUES (v_org, v_request_id, v_actor,
        'Deviation > 5% requires justification.', 'open')
  ...
  BEGIN
    IF v_clarification_id IS NOT NULL THEN
      ...
      SET justification = 'Vehicle ran extra route', justified_by = v_actor, justified_at = now()
    ...
    ELSE
      t_step:='step12_2_resolve'; t_flow:='steps_6_12'; t_status:='SKIP'; t_detail:='nothing to resolve'; RETURN NEXT;
    END IF;
  EXCEPTION WHEN OTHERS THEN
