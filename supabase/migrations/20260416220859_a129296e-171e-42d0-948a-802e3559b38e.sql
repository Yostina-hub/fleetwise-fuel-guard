...
BEGIN
  ...
  BEGIN
    ...
    ) VALUES (v_org, v_wo_id, 'fleet_manager', v_actor, 'pending', 7500)
  ...
  BEGIN
    UPDATE public.fuel_work_orders SET emoney_amount = 7500, emoney_used = 0, updated_at = now() WHERE id = v_wo_id;
  ...
  BEGIN
    ...
    END IF;
    SELECT count(*) INTO v_nearby_count FROM public.get_nearby_fuel_stations(9.0192, 38.7525, 50, 'diesel', 0);
  ...
  -- Step 11: Transaction on fulfillment
  BEGIN
    UPDATE public.fuel_requests SET status = 'fulfilled', actual_liters = 102, fulfilled_at = now()
    WHERE id = v_request_id;
    ...
    ELSE
      t_step:='step11_transaction'; t_flow:='steps_6_12'; t_status:='WARN'; t_detail:='no auto txn'; RETURN NEXT;
    END IF;
  EXCEPTION WHEN OTHERS THEN
  ...
  -- Step 12.1: Deviation -> clarification
  BEGIN
    UPDATE public.fuel_requests SET actual_liters = 130, status = 'fulfilled', fulfilled_at = now()
    WHERE id = v_request_id;
  ...
  BEGIN
    IF v_clarification_id IS NOT NULL THEN
      ...
      SET status = 'resolved', resolution_notes = 'E2E justified', resolved_by = v_actor, resolved_at = now()
