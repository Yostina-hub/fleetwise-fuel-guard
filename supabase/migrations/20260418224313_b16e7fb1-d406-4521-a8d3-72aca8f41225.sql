    ...
    "fields": [
      { "id": "sec_meta", "key": "_sec_meta", "type": "section", "label": "Workflow context",
        "fields": [
          ...
          { "id": "f_vehicle", "key": "vehicle_id", "type": "vehicle", "label": "Vehicle", "required": true,
            "layout": {"colSpan": 2},
            "helpText": "Selecting a vehicle auto-fills identity, inspection dates and current odometer below." }
        ]
      },
      { "id": "sec_header", "key": "_sec_header", "type": "section", "label": "Header (EFM/FA/03)",
        "fields": [
          { "id": "f_3rd", "key": "third_party_inspection_expiry", "type": "date",
            ...
            "readOnly": true,
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "insurance_expiry"},
          ...
          { "id": "f_ann", "key": "annual_inspection_expiry", "type": "date",
            ...
            "readOnly": true,
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "registration_expiry"},
      ...
      { "id": "sec_identity", "key": "_sec_identity", "type": "section",
        "label": "Vehicle identity (auto-filled, read-only)",
        "fields": [
          { "id": "f_vt", "key": "vehicle_type", "type": "text",
            "label": "1. Type of vehicle", "layout": {"colSpan": 1},
            "readOnly": true,
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "vehicle_type"} },
          { "id": "f_vm", "key": "vehicle_model", "type": "text",
            "label": "2. Model", "layout": {"colSpan": 1},
            "readOnly": true,
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "model"} },
          { "id": "f_ch", "key": "chassis_no", "type": "text",
            "label": "3. Chassis No. (VIN)", "layout": {"colSpan": 1},
            "readOnly": true,
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "vin"} },
          { "id": "f_en", "key": "engine_no", "type": "text",
            "label": "4. Engine No.", "layout": {"colSpan": 1},
            "helpText": "Manual entry — not stored on the vehicle record." },
          { "id": "f_pl", "key": "plate_no", "type": "text",
            "label": "5. Plate No.", "layout": {"colSpan": 1},
            "readOnly": true,
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "plate_number"} },
          { "id": "f_cap", "key": "passenger_load_capacity", "type": "text",
            "label": "6. Passenger / Load capacity (kg)", "layout": {"colSpan": 1},
            "readOnly": true,
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "capacity_kg"} },
          { "id": "f_km", "key": "km_reading", "type": "number",
            "label": "7. KM reading", "required": true, "layout": {"colSpan": 1},
            "autofillFrom": {"entity": "vehicle", "sourceKey": "vehicle_id", "sourceField": "odometer_km"},
            "helpText": "Pre-filled from last known odometer — update to actual reading at handover." },
      ...
      { "id": "sec_acc", "key": "_sec_acc", "type": "section",
        ...
        "fields": [
          { "id": "f_lines", "key": "checklist_lines", "type": "repeater",
            ...
            "helpText": "Pick each item from the configurable catalog (managed in /sop/vehicle-handover/catalog). Use \"+ Custom item\" for ad-hoc additions.",
            "layout": {"colSpan": 2},
            "fields": [
              { "id": "rl_name", "key": "name", "type": "catalog_picker",
                "label": "List of material", "layout": {"colSpan": 1},
                "catalog": "vehicle_handover", "allowCustom": true,
                "placeholder": "Select item…" },
              { "id": "rl_qty",  "key": "qty",  "type": "number", "label": "QTY", "layout": {"colSpan": 1} },
              { "id": "rl_cond", "key": "condition", "type": "select",
                "label": "Condition", "layout": {"colSpan": 2},
                "options": [
      ...
      { "id": "sec_signoff", "key": "_sec_signoff", "type": "section", "label": "Three-party identity",
        "fields": [
          ...
          { "id": "f_dp", "key": "delivered_by_phone", "type": "phone",
            "label": "Delivered by — Phone", "layout": {"colSpan": 2},
            "helpText": "Used for SMS / WhatsApp confirmation when the handover is archived." },
          ...
          { "id": "f_rp", "key": "received_by_phone", "type": "phone",
            "label": "Received by — Phone", "layout": {"colSpan": 2} },
...
BEGIN
  FOR v_org IN SELECT id FROM public.organizations LOOP
    -- Find the existing form (seeded by the previous migration)
    SELECT id, current_published_version_id
      INTO v_form_id, v_old_version_id
    FROM public.forms
    ...
    IF v_form_id IS NULL THEN
      CONTINUE; -- no seed yet for this org; previous migration handled creation
    END IF;

    -- Pick a creator (super_admin/fleet_manager preferred)
    ...
    -- Skip if a v2+ already exists with the same schema (idempotent re-runs)
    IF EXISTS (
    ...
    VALUES (v_form_id, v_org.id, v_next_version, 'published', v_schema, v_settings, now(), v_creator, v_creator)
