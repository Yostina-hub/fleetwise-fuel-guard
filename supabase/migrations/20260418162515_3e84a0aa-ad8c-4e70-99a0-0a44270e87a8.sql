...
BEGIN
  ...
  -- Prefer the ASSIGNED approver as the "performer" of the routing decision,
  -- since that is who is responsible for the next step. Fall back to the actor
  -- (the user who triggered the routing) if no assignee is recorded.
  ...
  -- Try to extract assignee user id from new_values JSON
  BEGIN
  ...
  -- Try assignee email/name embedded directly in new_values
  ...
  -- Final fallback: actor who triggered the routing
  IF v_perf_id IS NULL THEN v_perf_id := NEW.actor_id; END IF;
