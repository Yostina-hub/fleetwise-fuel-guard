...
CREATE OR REPLACE FUNCTION public.e2e_set_user(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.e2e_set_user(uuid) FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION public.e2e_check_stage(p_req uuid, p_expected text)
...
DECLARE r record;
BEGIN
  SELECT workflow_stage INTO r FROM public.maintenance_requests WHERE id = p_req;
  IF r.workflow_stage = p_expected THEN
    RETURN QUERY SELECT 'PASS'::text, ('stage='||p_expected)::text;
  ELSE
    RETURN QUERY SELECT 'FAIL'::text, ('expected '||p_expected||' got '||COALESCE(r.workflow_stage,'<null>'))::text;
  END IF;
END;
$$;

REVOKE ALL ON FUNCTION public.e2e_check_stage(uuid, text) FROM PUBLIC, anon, authenticated;
...
BEGIN
  DELETE FROM public.maintenance_workflow_events
   WHERE request_id IN (SELECT id FROM public.maintenance_requests WHERE request_number LIKE 'E2E-%');
  DELETE FROM public.maintenance_requests WHERE request_number LIKE 'E2E-%';
  DELETE FROM public.geofences WHERE name = 'E2E Supplier Workshop' AND organization_id = v_org;
  DELETE FROM public.vehicle_telemetry
    WHERE vehicle_id = v_vehicle AND organization_id = v_org
      AND (latitude BETWEEN 8.97 AND 9.20) AND (longitude BETWEEN 38.74 AND 38.95);
  RETURN 'cleanup complete';
END;
$$;

REVOKE ALL ON FUNCTION public.cleanup_maintenance_workflow_e2e_test() FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  INSERT INTO public.geofences (organization_id, name, geometry_type, center_lat, center_lng, radius_meters, is_active, category, created_by)
  VALUES (v_org, 'E2E Supplier Workshop', 'circle', v_geo_lat, v_geo_lng, 200, true, 'supplier', v_scd)
  ...
  -- ============ FLOW B: UNVERIFIED ============
  INSERT INTO public.vehicle_telemetry (vehicle_id, organization_id, latitude, longitude, last_communication_at, device_connected)
  VALUES (v_vehicle, v_org, v_outside_lat, v_outside_lng, now(), true);
