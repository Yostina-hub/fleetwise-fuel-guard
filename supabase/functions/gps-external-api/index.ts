...
async function processRecord(supabase: any, record: TelemetryRecord): Promise<ProcessResult> {
  ...
  if (device.vehicle_id) {
    ...
    if (speedKmh > 0) {
      ...
      if (isGovernorActive && record.latitude && record.longitude) {
        ...
        if (zones && zones.length > 0) {
          for (const zone of zones) {
            ...
            if (geofence?.center_lat && geofence?.center_lng && geofence?.radius_meters) {
              const distance = calculateDistance(record.latitude, record.longitude, geofence.center_lat, geofence.center_lng) * 1000;
        ...
        if (geofencesWithSpeed && geofencesWithSpeed.length > 0) {
          for (const gf of geofencesWithSpeed) {
            if (gf.center_lat && gf.center_lng && gf.radius_meters && gf.speed_limit) {
              const distance = calculateDistance(record.latitude, record.longitude, gf.center_lat, gf.center_lng) * 1000;
      ...
      if (isGovernorActive && speedKmh > speedLimit) {
        ...
        if (!recentViolation) {
          ...
          if (violationError) {
            ...
            console.log(`Speed violation recorded: ${speedKmh} km/h (limit: ${speedLimit}${zoneName ? ` in ${zoneName}` : ''})`);
          }
          ...
          const alertMessage = `${vehicleData?.plate_number || 'Vehicle'} traveling at ${speedKmh.toFixed(0)} km/h (limit: ${speedLimit} km/h)${zoneName ? ` in ${zoneName}` : ''}`;
