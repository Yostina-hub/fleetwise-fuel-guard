...
export const DocumentExpiryTable = ({ documents }: DocumentExpiryTableProps) => {
  ...
  // Filter documents with expiry dates and sort by urgency
  const expiringDocs = documents
    .filter(d => d.expiry_date)
    .map(d => ({
      ...d,
      daysUntilExpiry: differenceInDays(new Date(d.expiry_date!), today),
      isExpired: isBefore(new Date(d.expiry_date!), today),
    }))
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground/70 mt-1">
            No documents with expiration tracking found
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          Document Expiry Report ({expiringDocs.length})
        </CardTitle>
      </CardHeader>
            ...
            <tbody className="divide-y divide-border">
              {expiringDocs.map((doc) => (
                <tr key={doc.id} className={cn(
                  ...
                  doc.isExpired && "bg-red-500/5"
                )}>
                  <td className="px-4 py-3 font-medium text-sm max-w-xs truncate">
                    {doc.file_name}
                  </td>
                  <td className="px-4 py-3 capitalize">
                    {doc.document_type.replace(/_/g, " ")}
                  </td>
                  <td className="px-4 py-3 capitalize text-sm text-muted-foreground">
                    {doc.entity_type}
                  </td>
