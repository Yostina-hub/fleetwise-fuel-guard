      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground/70 mt-1">
            No trips with idle time recorded in this period
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          Idle Time Analysis ({tripsWithIdle.length} trips)
        </CardTitle>
      </CardHeader>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-border">
              {tripsWithIdle.map((trip) => {
                const idlePercent = trip.duration_minutes 
                  ? ((trip.idle_time_minutes || 0) / trip.duration_minutes) * 100 
                  : 0;
                  ...
                  <tr key={trip.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {(trip.idle_time_minutes || 0) > 30 && (
                          <AlertCircle className="w-4 h-4 text-amber-500" />
                        )}
                        <span className={cn(
                          "font-medium",
                          (trip.idle_time_minutes || 0) > 60 ? "text-red-500" :
                          (trip.idle_time_minutes || 0) > 30 ? "text-amber-500" :
                          "text-foreground"
                        )}>
                          {trip.idle_time_minutes}m
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={cn(
                        "font-medium",
                        idlePercent > 30 ? "text-red-500" :
                        idlePercent > 15 ? "text-amber-500" :
                        "text-green-500"
                      )}>
