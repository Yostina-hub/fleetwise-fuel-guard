...
const FleetStatusDonut = ({ vehicles, loading }: FleetStatusDonutProps) => {
  ...
  const statusData = useMemo(() => {
    ...
    return [
      { name: t('executive.active'), value: active, color: "hsl(var(--success))", percentage: vehicles.length > 0 ? (active / vehicles.length * 100) : 0 },
      { name: t('executive.maintenance'), value: maintenance, color: "hsl(var(--warning))", percentage: vehicles.length > 0 ? (maintenance / vehicles.length * 100) : 0 },
      { name: t('executive.inactive'), value: inactive, color: "hsl(var(--muted-foreground))", percentage: vehicles.length > 0 ? (inactive / vehicles.length * 100) : 0 },
    ];
  }, [vehicles, t]);
      ...
      <CardContent>
        <div className="relative h-[200px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={statusData} cx="50%" cy="50%" innerRadius={60} outerRadius={85} paddingAngle={4} dataKey="value" animationDuration={1500} animationBegin={0}>
                {statusData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.color} className="drop-shadow-lg" style={{ filter: `drop-shadow(0 0 6px ${entry.color})` }} />
                ))}
              </Pie>
              <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '12px' }} formatter={(value: number, name: string) => [`${value} ${t('executive.vehicles').toLowerCase()}`, name]} />
            </PieChart>
          </ResponsiveContainer>
