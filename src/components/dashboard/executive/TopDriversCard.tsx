...
const TopDriversCard = ({ drivers, loading }: TopDriversCardProps) => {
  ...
  if (loading) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl bg-surface-elevated border border-surface-border p-6">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative rounded-2xl bg-surface-elevated border border-surface-border overflow-hidden">
      ...
      <div className="relative z-10 p-6">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-surface-muted-foreground">{t('executive.bySafetyPerformance')}</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          {drivers.slice(0, 5).map((driver, index) => (
            <motion.div key={driver.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="flex items-center gap-3 p-3 rounded-xl bg-surface-card hover:bg-surface-card/80 transition-colors group">
              <div className="w-8 h-8 rounded-lg bg-surface-overlay flex items-center justify-center">{getRankIcon(driver.rank)}</div>
              <Avatar className="h-10 w-10 border-2 border-surface-border shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="relative h-1.5 bg-surface-overlay rounded-full overflow-hidden">
                ...
                <p className="text-xs text-surface-muted-foreground mt-1">{driver.totalTrips} {t('executive.trips')} • {driver.totalDistance.toLocaleString()} km</p>
              </div>
            </motion.div>
          ))}
        </div>
