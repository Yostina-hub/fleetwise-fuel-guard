...
export const TripConsolidationWorkspace = ({ organizationId }: Props) => {
  ...
  const orderedStops = useMemo<Stop[]>(() => {
    ...
    // Start from the request with the earliest needed_from time.
    ...
    // Seed: first pickup of earliest request.
    const seed = sorted[0];
    stops.push({
      type: "pickup",
    ...
    // Greedy nearest-neighbor: at each step choose nearest unpicked pickup
    // OR drop of an already-picked-up request. A request can only drop after
    // its pickup has been visited.
  ...
  // Fetch combined route from edge function whenever the ordered stops change.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!error && data?.ok && Array.isArray(data.geometry) && data.geometry.length >= 2) {
          ...
          // Fallback: connect stops in straight lines
          setCombinedRoute({
            geometry: coords,
            distance_m: 0,
            duration_s: 0,
